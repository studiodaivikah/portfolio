...
export async function GET(request: NextRequest) {
  try {
    ...
    const blog = await prisma.blog2.findUnique({
      where: { projectId: id }
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingBlog = await prisma.blog2.findUnique({
      where: { projectId: id }
    });
    ...
    if (existingBlog) {
      ...
      blog = await prisma.blog2.update({
        where: { projectId: id },
        data: {
          paragraphs,
          images: images || [],
      ...
      // Create new blog
      blog = await prisma.blog2.create({
        data: {
          projectId: id,
          paragraphs,
          images: images || [],
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleted = await prisma.blog2.delete({
      where: { projectId: id }
    });
